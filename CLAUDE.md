...
- **Seção "Evolução por dia"** (ordem na página: logo depois dos cards de Tributação/Departamento, **acima** de "Pendências por Gerente de Contas"): gráfico de barra empilhada por dia (SVG desenhado à mão, sem lib externa — bar em vez de linha porque é uma contagem discreta por dia, não um total acumulado) mostrando quantas empresas foram confirmadas por dia (`DataConfirmacao`, coluna do Radar Fiscal), separado por Documentação Recebida/Pendente (Recebida embaixo, Pendente empilhada em cima). `contarConfirmacoesPorDia()` + `renderizarEvolucao()` em `static/script.js`, recalculado dentro de `aplicarFiltros()` — **usa a data que já está em cada linha da planilha**, não um snapshot acumulado por execução do robô (decisão do usuário 2026-08-18, depois de eu ter implementado a versão por snapshot/`historico.json` e ele preferir os dados reais da planilha). Por isso reage aos filtros normalmente, igual aos cards/ranking. Exige pelo menos 2 dias distintos com `DataConfirmacao` no filtro atual — antes disso mostra uma mensagem.
  - **Em aberto**: perguntei ao usuário se essa seção precisa de um filtro de data/semana próprio. Minha avaliação foi que ainda não é necessário — a `DataConfirmacao` fica presa ao ciclo de fechamento atual (hoje variando só ~03/08 a 17/08) e o gráfico já decima os rótulos de eixo quando há muitos dias — mas faria sentido se o histórico passar a acumular vários meses. Usuário não decidiu ainda; não implementar sem confirmação.
