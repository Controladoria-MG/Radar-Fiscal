...
### Cópia de rede (Y:)
- Ao final de cada exportação bruta, o robô também copia o arquivo para `Y:\GERÊNCIA\Gerencia Operacional\CTR\Juan\Controle de Fechamentos\` (`Radar Fiscal\radar_fiscal.xlsx` e `Planilha de Mercados\planilha_mercados.xlsx`) — `_copiar_para_rede()` em `backend/radar_fiscal.py`.
- Essas cópias alimentam a Power Query do arquivo `Resumo Radar Fiscal.xlsx` (fora deste repo, na mesma pasta de rede) — é a origem histórica do projeto e continua sendo mantido pelo próprio usuário/Excel, refletindo essas duas planilhas ao ser atualizado. **Não precisa ser gerado nem tocado por este robô.**
- Falha ao copiar para a rede (ex.: unidade Y: indisponível) não interrompe a execução — só gera um aviso no log; `resumo.xlsx` e o portal continuam sendo gerados normalmente a partir das cópias locais em `data/radar_fiscal/`.
...
- **Cards de tamanho uniforme**: todo card sempre renderiza as duas linhas de Documentação (Recebida/Pendente) e os 5 status na mesma ordem fixa (`STATUS_ORDEM` em `static/script.js`), mesmo com contagem 0 — decisão consciente do usuário (2026-08-18) pra manter todos os cards do mesmo tamanho/altura em vez de encolher quando faltam categorias. `criarContadorDocs()`/`criarContadorStatus()` pré-populam os mapas zerados antes de contar.
- **Sem legenda no cabeçalho da seção de cards**: o chip "Documentação Recebida/Documentação Pendente" (`.quebra-legenda-fixa`) foi removido do cabeçalho de "Por Tributação"/"Por Departamento" a pedido do usuário (2026-08-18) — a cor já é auto-explicada dentro de cada card. A mesma legenda continua na seção "Evolução por dia" (abaixo), onde ainda é necessária pra identificar as cores da barra.
- **Seção "Evolução por dia"** (ordem na página: logo depois dos cards de Tributação/Departamento, **acima** de "Pendências por Gerente de Contas"): gráfico de uma barra por dia (SVG desenhado à mão, sem lib externa — bar em vez de linha porque é uma contagem discreta por dia, não um total acumulado) mostrando quantas empresas foram fechadas por dia (`DataConfirmacao`, coluna do Radar Fiscal) — total único por dia, **sem** separar por Documentação Recebida/Pendente (removido a pedido do usuário em 2026-08-19; a barra empilhada e a legenda que a acompanhava foram tiradas, `.evolucao-barra` agora é uma cor só). Todo dia do histórico mostra sua data no eixo — sem decimar rótulo (também a pedido do usuário). `contarConfirmacoesPorDia()` + `renderizarEvolucao()` em `static/script.js`, recalculado dentro de `aplicarFiltros()` — **usa a data que já está em cada linha da planilha**, não um snapshot acumulado por execução do robô (decisão do usuário 2026-08-18, depois de eu ter implementado a versão por snapshot/`historico.json` e ele preferir os dados reais da planilha). Por isso reage aos filtros normalmente, igual aos cards/ranking. Exige pelo menos 2 dias distintos com `DataConfirmacao` no filtro atual — antes disso mostra uma mensagem.
- **Sem filtro de período nessa seção**: cheguei a implementar um filtro de semana (abas segunda-domingo, 2026-08-19) e o usuário pediu pra tirar no mesmo dia — a seção sempre mostra o histórico completo do filtro atual (`filtrados`), sem período próprio. Não reintroduzir sem o usuário pedir de novo. O seletor `el.quebraAbas` continua escopado a `#quebra-abas-dimensao` (não removi esse fix, só o filtro de semana em si).
